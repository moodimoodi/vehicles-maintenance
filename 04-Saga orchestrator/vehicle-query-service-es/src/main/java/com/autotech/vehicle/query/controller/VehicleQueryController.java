...
public class VehicleQueryController {
    ...
    @GetMapping("/{vin}")
    public ResponseEntity<VehicleViewResponseDto> getByVin(@PathVariable String vin) {
        return ResponseEntity.ok(service.getByVin(vin));
    }

    @GetMapping("/owner/{ownerId}")
    public ResponseEntity<List<VehicleViewResponseDto>> getByOwner(@PathVariable String ownerId) {
        return ResponseEntity.ok(service.getByOwner(ownerId));
    }

    @GetMapping("/status/{status}")
    public ResponseEntity<List<VehicleViewResponseDto>> getByStatus(@PathVariable String status) {
